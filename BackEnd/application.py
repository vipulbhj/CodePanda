...
def submission():
    ...
    if request.method == 'POST':
        fullFilename = request.form.get('filename') + request.form.get('extension')
        code = request.form.get('code')

        if request.form.get('extension') == '.java':
            try:
                match = re.compile(r'(public)? class (.+){')
        ...
        if request.form.get('extension') == '.py':
            ...
            os.system('./submissions/cScript.sh ' + fullFilename + ' ' + className)
        else:
